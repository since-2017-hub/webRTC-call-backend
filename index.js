...
app.use((req, res, next) => {
  res.setHeader("Referrer-Policy", "no-referrer"); // or any other value
  next();
});
app.use(morgan("dev"));
...
io.on("connection", (socket) => {
  ...
  socket.on("join", (userData) => {
    ...
    console.log(
      `${user.username} is now online. Total online users: ${activeUsers.size}`
    );
  ...
  socket.on("end_call", (data) => {
    ...
    if (call) {
      const otherUserId =
        call.from.id === socket.userId ? call.to.id : call.from.id;
